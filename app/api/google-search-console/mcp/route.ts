...
import { comparePostPerformance, getPostPerformance } from "@/modules/google-search-console/reporting";
...
function errorResult(error: unknown) {
  ...
  console.error("[google-search-console-mcp] tool call threw an unexpected error:", error);
  return {
...
async function verifyToken(_req: Request, bearerToken?: string): Promise<AuthInfo | undefined> {
  ...
  try {
    ...
    const moduleEnabled = organisation ? await isModuleEnabled(organisation.organisationId, "google-search-console") : false;
    ...
    console.error("[google-search-console-mcp] auth threw an error:", error);
    return undefined;
  }
}
...
// elicitation notes. Only sites with an active Search Console mapping are offered,
// since an unmapped site has nothing this module can report on.
function resolveSite(ctx: ServerContext, siteId?: number): ResolvedSite {
  ...
  if (sites.length === 0) {
    throw new ToolError(
      "No WordPress site has an active Search Console mapping yet. Visit /google-search-console/connect to map one, then try again."
    );
  }
...
function reportErrorMessage(status: "not_mapped" | "consent_expired" | "temporary_failure" | "unavailable"): string {
  switch (status) {
    case "not_mapped":
      return "This site has no active Search Console mapping. Visit /google-search-console/connect to map one.";
    case "consent_expired":
      return "The connected Google account needs to be reconnected. Visit /google-search-console/connect to reconnect.";
    case "temporary_failure":
      return "Search Console is temporarily unavailable (rate limit or network issue). Try again shortly.";
    case "unavailable":
      return "Search Console data is not available for this request right now.";
  }
}
...
const rawHandler = createMcpHandler(
    ...
    const registerGatedTool: typeof server.registerTool = ((name: string, config: unknown, handler: (...a: unknown[]) => unknown) => {
      return server.registerTool(name, config as never, (async (...handlerArgs: unknown[]) => {
        try {
          ...
          assertModuleEnabled(moduleEnabled, "google-search-console");
        } catch (e) {
  ...
  },
  {
    instructions:
      ...
      "get_search_console_performance and compare_search_console_periods, so a value obtained from list_mapped_sites " +
      "can be reused across calls in the same session.",
  }
);

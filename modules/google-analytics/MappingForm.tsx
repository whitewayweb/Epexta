...
export function MappingForm({ wordpressConnections, googleConnections }: MappingFormProps) {
  ...
  if (wordpressConnections.length === 0) {
    return <p className="text-sm text-muted-foreground">Connect a WordPress site first, then map it to a GA4 property here.</p>;
  }
  ...
  return (
    <form action={formAction} className="flex max-w-md flex-col gap-4">
      ...
      <div className="grid gap-1.5">
        ...
        <Input id="ga4PropertyId" type="text" name="ga4PropertyId" required placeholder="properties/123456789" />
      </div>
